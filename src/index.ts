...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'US Travel Advisories');
}
...
async function fetchFeed(): Promise<string> {
  const res = await pwFetch(FEED, { headers: { Accept: 'application/xml, text/xml', 'User-Agent': UA } });
  if (!res.ok) throw await httpError(res, 'Travel advisories');
  return res.text();
}
